...
class _GenImgScreenState extends State<GenImgScreen> {
  ...
  void initState() {
    openAI = OpenAI.instance
        .build(token: kToken, baseOption: HttpSetup(receiveTimeout: const Duration(seconds: 6)), enableLog: true);
    super.initState();
  }
  ...
  void _generateImage() async {
    ...
    final request = GenerateImage(prompt, 1, model: DallE3(), size: ImageSize.size256, responseFormat: Format.url);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: Column(
        ...
        children: [
          Center(child: ElevatedButton(onPressed: () => _generateImage(), child: const Text("Generate Image"))),
          img == ""
              ? const Text("Loading...")
              : AspectRatio(
                  aspectRatio: 16 / 9,
