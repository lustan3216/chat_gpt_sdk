...
import 'package:chat_gpt_sdk_lululala/src/edit.dart';
import 'package:chat_gpt_sdk_lululala/src/embedding.dart';
import 'package:chat_gpt_sdk_lululala/src/fine_tuned.dart';
import 'package:chat_gpt_sdk_lululala/src/model/chat_complete/response/chat_choice.dart';
import 'package:chat_gpt_sdk_lululala/src/model/chat_complete/response/chat_choice_sse.dart';
import 'package:chat_gpt_sdk_lululala/src/model/complete_text/response/choices.dart';
import 'package:chat_gpt_sdk_lululala/src/model/complete_text/response/usage.dart';
import 'package:chat_gpt_sdk_lululala/src/model/edits/enum/edit_model.dart';
import 'package:chat_gpt_sdk_lululala/src/model/embedding/enum/embed_model.dart';
import 'package:chat_gpt_sdk_lululala/src/model/gen_image/response/image_data.dart';
import 'package:chat_gpt_sdk_lululala/src/model/moderation/enum/moderation_model.dart';
import 'package:chat_gpt_sdk_lululala/src/model/openai_engine/engine_data.dart';
import 'package:chat_gpt_sdk_lululala/src/model/openai_model/model_data.dart';
import 'package:chat_gpt_sdk_lululala/src/model/openai_model/permission.dart';
...
void main() async {
  ...
  test('description', () async {
    ...
    expect(
      () => ai.moderation.create(input: 'input', model: TextLastModerationModel()),
      throwsException,
    );
  ...
  group('chatGPT-3 text completion test', () {
    ...
    test('text completion error case with prompt empty', () async {
      ...
      when(openAI.onCompletion(request: request)).thenAnswer((_) => throw RequestError(data: null, code: 404));
  ...
  group('chatGPT-3 text completion with stream SSE test', () {
    test('text completion stream case success', () {
      final request = CompleteText(prompt: 'snake is ?', model: Babbage002Model());
      final choice = [
        Choices(
          '',
          1,
      ...
      when(openAI.onCompletionSSE(request: request)).thenAnswer((_) => Stream.value(
    ...
    test('text completion stream case error', () async {
      ...
      when(openAI.onCompletionSSE(request: request)).thenAnswer((_) => throw RequestError(data: null, code: 404));
  ...
  group('chatGPT-3.5 turbo chat completion test', () {
    test('chat completion success case test', () async {
      ...
      when(openAI.onChatCompletion(request: request)).thenAnswer((_) async => ChatCTResponse(
            id: 'id',
            object: 'object',
            created: 1,
            choices: choice,
    ...
    test(
      ...
      () {
        ...
        when(openAI.onChatCompletion(request: request)).thenAnswer((_) => throw RequestError(data: null, code: 404));
  ...
  group('chatGPT-3.5 turbo chat completion with stream SSE test', () {
    ...
    test('openAI chat completion stream error case test', () async {
      ...
      when(openAI.onChatCompletionSSE(request: request)).thenAnswer((_) => throw RequestError());
  ...
  group('chatGPT Image Generate With Prompt test case', () {
    test('chatGPT Image Generate With Prompt success case  test', () async {
      final request = GenerateImage('snake red eating cat.', model: DallE3(), 2);

      when(openAI.generateImage(request)).thenAnswer((_) async => GenImgResponse());
    ...
    test(
      ...
      () async {
        ...
        when(openAI.generateImage(request)).thenAnswer((_) async => GenImgResponse(created: 1221120));
    ...
    test(
      ...
      () async {
        final request = GenerateImage('snake red eating cat.', model: DallE3(), 1);
  ...
  group('chatGPT Models & Engine test', () {
    ...
    test('chatGPT Engine success case test', () async {
      when(openAI.listEngine())
          .thenAnswer((_) async => EngineModel([EngineData('id', 'object', 'owner', false)], 'object'));
  ...
  group('chatGPT Image Generate test', () {
    test(
      ...
      () async {
        ...
        when(openAI.generateImage(request)).thenAnswer((realInvocation) async => GenImgResponse(
              created: 912312,
  ...
  group('chatGPT edit test', () {
    test('chatGPT Edit image test with success case', () {
      ...
      when(edit.editImage(request))
          .thenAnswer((realInvocation) async => GenImgResponse(created: 123, data: [ImageData(url: "url")]));
    ...
    test('ChatGPT Edit image test with unauthenticated error case', () async {
      final request = EditImageRequest(image: FileInfo("path", "name"), prompt: "fix body");
    ...
    test('ChatGPT Edit image test with rate limit error case', () async {
      final request = EditImageRequest(image: FileInfo("path", "name"), prompt: "fix");
    ...
    test('chatGPT Edit prompt test with success case', () {
      ...
      when(edit.prompt(request)).thenAnswer((realInvocation) async => EditResponse(
            object: 'object',
            created: 1,
            choices: choice,
            usage: usage,
    ...
    test('chatGPT edit variation test with success case', () {
      ...
      when(edit.variation(request))
          .thenAnswer((realInvocation) async => GenImgResponse(created: 1, data: [ImageData(url: "image url")]));
  ...
  group('chatGPT embedding test', () {
    test('chatGPT embedding test with success case', () async {
      final request = EmbedRequest(model: TextEmbeddingAda002EmbedModel(), input: 'input');
      final usage = Usage(1, 2, 3);

      when(embedding.embedding(request)).thenAnswer((realInvocation) async => EmbedResponse(
            object: 'object',
            data: [],
            model: 'model',
            usage: usage,
