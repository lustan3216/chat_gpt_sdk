...
import 'package:chat_gpt_sdk_lululala/src/model/audio/enum/audio_format.dart';
import 'package:chat_gpt_sdk_lululala/src/model/gen_image/request/file_info.dart';
...
class AudioRequest {
  ...
  Future<FormData> toJson() async {
    return FormData.fromMap({
      'file': (await File(file.path).exists()) ? await MultipartFile.fromFile(file.path, filename: file.name) : null,
      'prompt': prompt,
      "model": "whisper-1",
      'response_format': responseFormat == null ? AudioFormat.json.getName() : responseFormat?.getName(),
